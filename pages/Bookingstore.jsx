...
const Booking = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      const bookedProperty = properties.find((property) => property.id === parseInt(id));
    if (bookedProperty) {
      const existingProperty = cart.find((item) => item.id === bookedProperty.id);
      if (existingProperty) {
        existingProperty.quantity += 1;
      } else {
        cart.push({ ...bookedProperty, quantity: 1, bookingDate: bookingDate });
      }
      setCart([...cart]);
    }
  ...
  return (
    <div className="container">
      ...
      <div className="entry" style={{ textAlign: 'center', width: '99%', padding: '5px', margin: '20px 0', backgroundColor: '#f7f7f7', borderRadius: '10px' }}>
